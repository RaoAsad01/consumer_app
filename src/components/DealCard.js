 ...
 *
 * @param {Object} props
 * @param {Object} props.item - Contains id, title and deals array
 * @param {string} props.item.id - Category ID (e.g., 'black-friday', 'cyber-monday') - IMPORTANT for navigation
 * @param {string} props.item.title - Card header title (e.g., "Black Friday Deals")
 * @param {Array} props.item.deals - Array of deal items to display
 * @param {Function} props.onPress - Callback when a deal item is pressed
 * @param {Function} props.onViewAllPress - Callback when "View All Offers" is pressed - receives {id, title, deals}
...
const DealCard = ({
  ...
  // The ID is crucial for navigation to the correct tab
  const cardId = item?.id || 'black-friday';
  ...
  const handleViewAllPress = () => {
    if (onViewAllPress) {
      // Pass the category ID so we can navigate to the correct tab in BlazingDealsScreen
      onViewAllPress({
        id: cardId,
        title: cardTitle,
        deals: displayDeals
